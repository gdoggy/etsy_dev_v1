...
func (s *AIService) GeneratePayloadContent(ctx context.Context, proxy *model.Proxy, keyword, extraPrompt, imgUrl string) (*gemini.AIGenResult, error) {
	// 准备返回结果
	result := &gemini.AIGenResult{
		Images:   make([]string, 0),
		TextSets: make([]gemini.TextSetResult, 0),
	}
...
func (s *AIService) generateTextsSDK(ctx context.Context, proxy *model.Proxy, keyword, productDesc, extraPrompt string) ([]gemini.TextSetResult, error) {
	client, err := s.createGenAIClient(ctx, proxy)
	if err != nil {
		return nil, err
	}
